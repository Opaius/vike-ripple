...
import { isBackend, remult, SubscriptionChannel } from 'remult';
...
function roomFromParam(
	key: (typeof ROOM_PARAM_KEYS)[number],
	value: string
): string {
	switch (key) {
		case 'userId':
...
export function extractLiveQueryUserId(channel: string): string | null {
	...
	const markerIndex = channel.indexOf(
		LIVE_QUERY_QUERY_MARKER,
		LIVE_QUERY_USER_PREFIX.length
	);
...
function getClient(): RemultPartySubscriptionClient | undefined {
	return (remult as any)?.apiClient?.subscriptionClient as
		| RemultPartySubscriptionClient
		| undefined;
}

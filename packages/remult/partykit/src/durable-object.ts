 ...
 */
export class RemultPartyRoom<Env extends Cloudflare.Env = any> extends Server<Env> {
		...
		return {
			resolveRoomId: (channel: string) => {
				if (channel.includes('?')) {
					...
					return params.get('roomId') || params.get('readerId') || params.get('userId') || 'global'
				}
				return 'global'
			},
			validateSubscription: (channel: string, roomName: string) => {
				...
				if (channel.includes('?')) {
					const params = new URLSearchParams(channel.split('?')[1])
					const partition = params.get('roomId') || params.get('readerId') || params.get('userId')
	...
	override async onStart() {
		...
		const storedConnections = await this.ctx.storage.get<Record<string, number>>('brotherConnections') || {}
		for (const b of this.brothers) {
			this.brotherConnections.set(b, storedConnections[b] ?? 0)
		}
		console.log(`[RemultPartyRoom] Started ${this.name} - sharded: ${this.sharded}, brothers: ${this.brothers.join(', ')}`)
	}
	...
	private async subscribeClient(connection: Connection, state: ConnectionState | undefined, channel: string) {
		if (this.options?.validateSubscription) {
			const allowed = await this.options.validateSubscription(channel, this.name)
			if (!allowed) {
				connection.send(JSON.stringify({
					type: 'error',
					channel,
		...
		console.log('[RemultPartyRoom] subscribe', connection.id, 'channel:', channel)
	...
	private unsubscribeClient(connection: Connection, state: ConnectionState | undefined, channel: string) {
		const currentState = connection.state as ConnectionState | undefined
	...
	private broadcastToChannel(channel: string, message: RemultPartyMessage, excludeClientId?: string) {
		let sent = 0
		const connections = [...this.getConnections()]
		for (const conn of connections) {
			if (conn.id === excludeClientId) continue
			const state = conn.state as ConnectionState | undefined
			if (state && state.channels.includes(channel)) {
				try {
		...
		console.log('[RemultPartyRoom] broadcast channel:', channel, 'connections:', connections.length, 'sent:', sent)
	}
	...
	private reportToFather() {
		...
		this.ctx.waitUntil(
			fatherStub.fetch('http://dummy/shard-report', {
				method: 'POST',
	...
	private forwardDataToFather(data: RemultPartyMessage) {
		...
		const fatherStub = (this.env as any).RemultPartyRoom.get(fatherId)
		this.ctx.waitUntil(
			fatherStub.fetch(`http://dummy/publish?excludeShard=${encodeURIComponent(this.name)}`, {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({
					channel: data.channel,
				...
				console.error('[RemultPartyRoom] Failed to forward data to father:', err)
			})
		)
	}
	...
	private forwardSignalToFather(senderId: string, data: RemultPartyMessage) {
		...
		const fatherStub = (this.env as any).RemultPartyRoom.get(fatherId)
		this.ctx.waitUntil(
			fatherStub.fetch('http://dummy/signal-route', {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({
					recipientId: data.id,
				...
				console.error('[RemultPartyRoom] Failed to forward signal to father:', err)
			})
		)
	}
	...
	private async handlePublishRequest(request: Request): Promise<Response> {
		try {
			...
			if (body.channel && body.data !== undefined) {
				console.log(`[RemultPartyRoom] /publish on ${this.name} channel: ${body.channel} (exclude: ${excludeShard})`)
				...
				if (!isBrother && this.sharded) {
					for (const brotherName of this.brothers) {
						...
						const brotherId = (this.env as any).RemultPartyRoom.idFromName(brotherName)
						const brotherStub = (this.env as any).RemultPartyRoom.get(brotherId)
						this.ctx.waitUntil(
							brotherStub.fetch(`http://dummy/publish?excludeShard=${encodeURIComponent(excludeShard || '')}`, {
								method: 'POST',
								headers: { 'content-type': 'application/json' },
								body: JSON.stringify(body)
							}).catch((err: any) => {
								console.error(`[RemultPartyRoom] Failed to forward publish to brother ${brotherName}:`, err)
							})
						)
					}
				}
	...
	private async handleShardReportRequest(request: Request): Promise<Response> {
		try {
			const body = (await request.json()) as { shardName: string; connections: number }
			if (body.shardName && typeof body.connections === 'number') {
				...
				await this.ctx.storage.put('brotherConnections', storedConnections)
				console.log(`[RemultPartyRoom] Shard ${body.shardName} reported ${body.connections} connections`)
				return new Response('OK', { status: 200 })
			}
		} catch (err) {
	...
	private async handleSignalRouteRequest(request: Request): Promise<Response> {
		try {
			const body = (await request.json()) as { recipientId: string; senderId: string; channel: string; payload: unknown }
			if (body.recipientId && body.senderId) {
				...
				if (!isBrother) {
					for (const brotherName of this.brothers) {
						...
						const brotherStub = (this.env as any).RemultPartyRoom.get(brotherId)
						this.ctx.waitUntil(
							brotherStub.fetch('http://dummy/signal-route', {
								method: 'POST',
								headers: { 'content-type': 'application/json' },
								body: JSON.stringify(body)
							}).catch((err: any) => {
								console.error(`[RemultPartyRoom] Failed forwarding signal to brother ${brotherName}:`, err)
							})
						)
					}
				}
	...
	override async fetch(request: Request): Promise<Response> {
		...
		if (isWS) {
			...
			if (this.sharded) {
				...
				console.log(`[RemultPartyRoom] Routing WebSocket to brother: ${brotherName}`)
				const brotherId = (this.env as any).RemultPartyRoom.idFromName(brotherName)
				const brotherStub = (this.env as any).RemultPartyRoom.get(brotherId)
		...
		if (url.pathname === '/keepAlive' && request.method === 'POST') {
			try {
				...
				for (const id of body.ids) {
					...
					if (row) {
						this.state.storage.sql.exec(`UPDATE live_queries SET lastUsed = ? WHERE id = ?`, now, id)
					} else {
						unknown.push(id)
					}
				}

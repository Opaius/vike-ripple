# @cioky/vike-core — Quirks, Fixes & Caveats
...
**Problem**: Running `@cioky/vike-core setup` multiple times appends a new `?direct` CSS handler each time. The guard string `'Handle ?direct query param'` was never actually written into the patched code, so it never matched.
...
**Problem**: `@cioky/vike-core setup` patches `@ripple-ts/vite-plugin` with `@import "tailwindcss"`. `@cioky/vike-pandacss setup` tried to replace the *original* pattern — but it was already replaced by the tailwind patch. The script reported "Could not find target in Ripple plugin".

**Fix**: Detect existing `TW_PATCH_APPLY` marker and replace the entire tailwind block with the Panda CSS `@layer` block.

**Files**: `@cioky/vike-pandacss/src/setup.js`

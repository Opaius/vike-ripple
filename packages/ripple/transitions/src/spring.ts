...
export function springValues(
	...
	for (let i = 0; i <= frames; i++) {
		...
		if (zeta < 1) {
			norm =
				1 -
				decay * Math.cos(omega1 * t) -
				(zeta / Math.sqrt(1 - zeta * zeta)) * decay * Math.sin(omega1 * t);
		} else {
...
function parseTransform(val: string): TransformComponents {
	...
	if (val.startsWith('matrix')) {
		...
		if (nums.length === 6) {
			...
			comps.scaleX = Math.sqrt(
				nums[0] * nums[0] + nums[1] * nums[1] + nums[2] * nums[2]
			);
			comps.scaleY = Math.sqrt(
				nums[4] * nums[4] + nums[5] * nums[5] + nums[6] * nums[6]
			);
	...
	const translateMatch = val.match(
...
export function getSpringKeyframes(
	initial: any,
	animate: any,
